...
def plot_multiclass_roc_curve(true_labels, predicted_probabilities):
    ...
    # plt.savefig('pic/LLBNN_R.png', format='png', dpi=300)
    # plt.savefig('pic/LLBNN_R.pdf', format='pdf', dpi=300)
    # plt.savefig('pic/LLBNN_R.eps', format='eps', dpi=300)
...
class CombinedModel(nn.Module):
    def __init__(self, lstm_input_size1=3, lstm_hidden_size1=128, lstm_layers1=2,
                 lstm_input_size2=9, lstm_hidden_size2=64, lstm_layers2=3,
        ...
        combined_input_dim = lstm_hidden_size1 + lstm_hidden_size2
    ...
    def forward(self, data):
        ...
        combined_features = torch.cat((h_n1[-1], h_n2[-1]), dim=1)
...
# class CombinedModel(nn.Module):
#     def __init__(self, lstm_input_size1=3, lstm_input_size2=9, hidden_size=128, num_layers=3, bayesian_model=None):
#         super(CombinedModel, self).__init__()
#         self.hidden_size = hidden_size
#         self.num_layers = num_layers
#
#         # 定义 LSTM 层
#         self.lstm1 = nn.LSTM(input_size=lstm_input_size1, hidden_size=hidden_size, num_layers=num_layers,
#                              batch_first=True)
#         self.lstm2 = nn.LSTM(input_size=lstm_input_size2, hidden_size=hidden_size, num_layers=num_layers,
#                              batch_first=True)
#
#         # 贝叶斯神经网络，拼接后输入维度为 2 * hidden_size
#         self.bayesian_nn = bayesian_model or BayesianNN(input_dim=2 * hidden_size)
#
#     def forward(self, data):
#         # 获取 batch size
#         batch_size = data.size(0)
#
#         # 初始化 LSTM 隐藏状态和记忆状态
#         h0_1 = torch.zeros(self.num_layers, batch_size, self.hidden_size).to(data.device)
#         c0_1 = torch.zeros(self.num_layers, batch_size, self.hidden_size).to(data.device)
#         h0_2 = torch.zeros(self.num_layers, batch_size, self.hidden_size).to(data.device)
#         c0_2 = torch.zeros(self.num_layers, batch_size, self.hidden_size).to(data.device)
#
#         # 拆分数据：data[:, :, :3] 是坐标数据，data[:, :, 3:] 是旋转矩阵数据
#         coords = data[:, :, :3]
#         rotation_matrices = data[:, :, 3:]
...
# 实例化模型
# device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# bayesian_model = BayesianNN(input_dim=256, hidden_dim=128, output_dim=3)  # 输入和隐藏维度可以在这里修改
# net = CombinedModel(lstm_input_size1=3, lstm_input_size2=9, hidden_size=128, num_layers=2,
...
for epoch in range(epochs):
    ...
    if average_valid_accuracy > best_acc:
        ...
        torch.save(net, 'pth/LLB_model.pt')
...
# plt.savefig('pic/LLBNN_C.png', format='png', dpi=300)
# plt.savefig('pic/LLBNN_C.pdf', format='pdf', dpi=300)
# plt.savefig('pic/LLBNN_C.eps', format='eps', dpi=300)
plt.show()